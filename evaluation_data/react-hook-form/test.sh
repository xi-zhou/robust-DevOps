...
pnpm run start & sleep 120  
echo "sleeping until server is up"
...
for i in $(seq 1 $EXECUTIONS); do
    ...
    pnpm run e2e --reporter junit --reporter-options mochaFile=test-results/results_[hash].xml
    jrm test-results/results_final.xml "test-results/*.xml"
    mv "/react-hook-form/test-results/results_final.xml" "/test-results/results_$i.xml"
    rm -r test-results/*
