...
for i in $(seq 1 $EXECUTIONS); do
    ...
    node browser-test-harness.js yarn test:browser:ci --record false --reporter junit --reporter-options mochaFile=test-results/results_[hash].xml
    jrm test-results/results_final.xml "test-results/*.xml"
    mv "/react-beautiful-dnd/test-results/results_final.xml" "/test-results/results_$i.xml"
    rm -r test-results/*
